...
The app and kext extensions are compatible will all macOS versions released so far (Intel CPUs with Turbo Boost, of course). The app is tested agains all betas before final versions are released, so if any incompatiblity is found in the future, will be reported here and on the blog, like when El Capitan was released.
